...
var splitVals = function splitVals(str) {
    var str = str.split(',');
...
var stringifyIt = function stringifyIt(arrayOrObj, key) {
    var tempArr = [];
    if (Array.isArray(arrayOrObj)) {
        // If this is an array...
        arrayOrObj.map(function (item) {
...
var toggleSpinner = function toggleSpinner(btn) {
...
prefForm.on('submit', function (e) {
    ...
    if ('hobbies' in prefs) prefs.hobbies = splitVals(prefs.hobbies.toLowerCase());

    if ('age' in prefs) prefs.age = parseInt(prefs.age);

    var prefsJson = JSON.stringify(prefs);

    // Check if object is empty
    if (prefsJson == '{}') return alert('Please select at least one preference.');
...
proForm.on('submit', function (e) {
    ...
    var prefs = {};
    $(this).find('[data-pref]').each(function () {
        var val = $(this).val(),
            pref = $(this).attr('data-pref');
        if (val != "") {
            prefs[pref] = val;
    ...
    $.ajax({
        ...
        if (msg.statusCode != 1) return alert('An error occurred. Please try again later.');
